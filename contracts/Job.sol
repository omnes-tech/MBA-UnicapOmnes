...
contract Job is ERC721A, Pausable, Ownable, IMentoring {
    ...
    // Mentoring setting
    uint256 public priceSign; // payment sign
    ...
    function interest(
        uint256 _definiteHours,
        string memory _name,
        string memory _email,
        string memory _socialnetwork
    ) external payable {
        require(msg.value >= priceSign, "wrong price sign");
        interestList[msg.sender] = aboutMentoring(
            _definiteHours,
            _name,
            _email,
            _socialnetwork,
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (!compareStrings(idURIs[tokenId], baseuRI)) {
            //colocar de arquivo com o final "/" = https://ipfs.io/ipfs/bafybeifiuedtb5rry2dnlh7ouk2fxaiemo6rfrrhj2haphd4zo4osbkjr4/
            return
                bytes(idURIs[tokenId]).length != 0
    ...
    function mentoringData(address _mentored)
        external
        view
        override
        onlyOwner
        returns (aboutMentoring memory)
    {
    ...
    modifier MentoringCompliance() {
        ...
        require(
            msg.value >= price * interestList[msg.sender].definiteHours - discont,
            "wrong price"
        );
